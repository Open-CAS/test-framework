...
def install_opencas():
    ...
    TestProperties.executor.execute(f"if [ -d {opencas_repo_name} ]; then rm -rf {opencas_repo_name}; fi")
    output = TestProperties.executor.execute("git clone --recursive https://github.com/Open-CAS/open-cas-linux.git")
    if output.exit_code != 0:
        raise Exception(f"Error while cloning repository: {output.stdout}\n{output.stderr}")

    LOGGER.info("Open CAS make and make install.")
    output = TestProperties.executor.execute("cd open-cas-linux/ && ./configure && make")
    if output.exit_code != 0:

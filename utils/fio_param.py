...
from utils.fio_result import FioResult
...
class IoEngine(Enum):
    ...
    psync = 2,
    # Basic readv or writev I/O. Will emulate queuing by coalescing adjacents IOs into a single submission.
    vsync = 3,
...
class VerifyMethod(Enum):
    ...
    sha1 = 2,
    # Verify a strict pattern. Normally fio includes a header with some basic information and checksumming, but if this
...
class FioParam(linux_comm.LinuxCommand):
    def __init__(self, fio, command_executor: executor.BaseExecutor, command_name='fio'):
        linux_comm.LinuxCommand.__init__(self, command_executor, command_name)
        if command_name == 'fio':
    ...
    def get_verification_pattern(self):
        if not self.verification_pattern:
            self.verification_pattern = '0x'
            for x in range(0, 4):
                self.verification_pattern += f'{random.randint(0, sys.maxsize):08x}'
            return self.verification_pattern

    def block_size(self, size: Size):
        return self.set_param('blocksize', int(size.get_value()))
    ...
    def file_size(self, size: Size):
        return self.set_param('filesize', int(size.get_value()))
    ...
    def ignore_errors(self, read_errors, write_errors, verify_errors):
        ...
        return self.set_param(
            'ignore_error',
            separator.join(str(err.value[0]) for err in read_errors),
            separator.join(str(err.value[0]) for err in write_errors),
            separator.join(str(err.value[0]) for err in verify_errors))
    ...
    def add_job(self, job_name=None):
        ...
        new_job = FioParam(self.fio, self.command_executor, f'[{job_name}]')

...
class Dut:
    ...
    def __str__(self):
        ...
        for disk in self.disks:
            str += f'\ttype: {disk["type"]}, serial: {disk["serial"]}, size: {disk["size"]}, blocksize: {disk["blocksize"]}\n'
        str += f'ipmi: {self.ipmi}\n' if self.ipmi is not None else ''
        str += f'spider: {self.spider}\n' if self.spider is not None else ''
        str += f'wps: {self.wps}\n' if self.wps is not None else ''
        str += f'wps port: {self.wps_port}\n' if self.wps_port is not None else ''
        return str

...
class Fio:
    def __init__(self, executor_obj):
        ...
        self.fio_file = ''
        self.jobs = []
        self.fio_result_dir = 'fioResults'
        self.executor = executor_obj
        self.base_cmd_parameters: utils.fio_param.FioParam = None  # fio_param
        self.global_cmd_parameters: utils.fio_param.FioParam = None  # fio_param

    def create_command(self):
        self.command = ''
        self.base_cmd_parameters = utils.fio_param.FioParam(self, self.executor, 'fio')
        self.global_cmd_parameters = utils.fio_param.FioParam(self, self.executor, '[global]')
    ...
    def calculate_timeout(self):
        ...
        run_time = self.global_cmd_parameters.get_parameter_value("runtime")
        if run_time is not None and run_time != 0:
            raise ValueError(
                "FIO parameter 'time_based' is set but parameter 'runtime' is missing or 0. Set 'runtime' parameter.")
    ...
    def __str__(self):
        if len(self.jobs) > 0:
            self.command = f"echo '{self.execution_cmd_parameters()}' | {str(self.base_cmd_parameters)} -"
        else:
            fio_parameters = utils.fio_param.FioParam(self, self.executor, 'fio')
            ...
            self.command = str(fio_parameters)
        return self.command

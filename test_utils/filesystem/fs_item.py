...
from test_tools.fs_tools import chmod_numerical, Permissions, PermissionsUsers, PermissionSign, \
    chmod, chown, check_if_directory_exists, copy, ls_item, parse_ls_output, move
...
class FsItem:
    ...
    def chmod_numerical(self, permissions: int, recursive: bool = False):
        chmod_numerical(self.full_path, permissions, recursive)
        self.refresh_item()
    ...
    def chmod(self,
        ...
        chmod(self.full_path, permissions, users, sign=sign, recursive=recursive)
        self.refresh_item()

    def chown(self, owner, group, recursive: bool = False):
        chown(self.full_path, owner, group, recursive)
        self.refresh_item()
    ...
    def copy(self,
        ...
        copy(str(self), destination, force, recursive, dereference)
    ...
    def move(self,
             ...
             force: bool = False):
        target_dir_exists = check_if_directory_exists(destination)
        move(str(self), destination, force)
        if target_dir_exists:
